...
local ElonmuskapiSDK = {}
ElonmuskapiSDK.__index = ElonmuskapiSDK
...
ElonmuskapiSDK._make_feature = _make_feature


function ElonmuskapiSDK.new(options)
  local self = setmetatable({}, ElonmuskapiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ElonmuskapiSDK:options_map()
...
function ElonmuskapiSDK:get_utility()
  return Utility.copy(self._utility)
end


function ElonmuskapiSDK:get_root_ctx()
  return self._rootctx
end


function ElonmuskapiSDK:prepare(fetchargs)
...
function ElonmuskapiSDK:direct(fetchargs)
...
-- by passing it.
function ElonmuskapiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ElonmuskapiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ElonmuskapiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ElonmuskapiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ElonmuskapiSDK.new(sdkopts)
  sdk.mode = "test"
