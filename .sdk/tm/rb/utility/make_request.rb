...
module ElonmuskapiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ElonmuskapiResponse.new({})
    ...
    if fetch_err
      ...
      response = ElonmuskapiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ElonmuskapiResponse.new(fetched)
    else
