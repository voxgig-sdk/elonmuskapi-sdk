...
class ElonmuskapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ElonmuskapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ElonmuskapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ElonmuskapiTestRunner.getenv("ELONMUSKAPI_TEST_LIVE")
        override = ElonmuskapiTestRunner.getenv("ELONMUSKAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ElonmuskapiTestRunner.getenv(key)
        ...
        explain = ElonmuskapiTestRunner.getenv("ELONMUSKAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ELONMUSKAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ElonmuskapiTestRunner._test_control is not None:
            return ElonmuskapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ElonmuskapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ElonmuskapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ElonmuskapiTestRunner.load_test_control()
...
def load_env_local():
    ElonmuskapiTestRunner.load_env_local()


def env_override(m):
    return ElonmuskapiTestRunner.env_override(m)


def entity_data(v):
    return ElonmuskapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ElonmuskapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ElonmuskapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ElonmuskapiTestRunner.load_test_control()


def live_client_options():
    return ElonmuskapiTestRunner.live_client_options()


def live_delay_ms():
    return ElonmuskapiTestRunner.live_delay_ms()
