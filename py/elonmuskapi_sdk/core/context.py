...
from elonmuskapi_sdk.core.control import ElonmuskapiControl
...
from elonmuskapi_sdk.core.helpers import get_ctx_prop, to_map
...
class ElonmuskapiContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, ElonmuskapiSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ElonmuskapiResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ElonmuskapiResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return ElonmuskapiError(code, msg, self)
