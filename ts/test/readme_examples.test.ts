...
//     (\`new ElonmuskapiSDK(...)\` / \`ElonmuskapiSDK.test(...)\` become
//     \`ElonmuskapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_random_article":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ElonmuskapiSDK(...)\` and any
// \`ElonmuskapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ElonmuskapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ElonmuskapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
