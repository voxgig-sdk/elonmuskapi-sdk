...
describe('GetRandomArticleDirect', async () => {
  ...
  test('direct-load-get_random_article', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_random_article', setup.live)) return
