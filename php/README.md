...
The PHP SDK for the Elonmuskapi API — an entity-oriented client using PHP conventions.
...
try {
    ...
    print_r($getrandomarticle->data_get());
} catch (\Throwable $err) {
...
```php
require_once 'elonmuskapi_sdk.php';
$client = new ElonmuskapiSDK($options);
```
...
```
php/
├── elonmuskapi_sdk.php          -- Main SDK class
...
The main class (`elonmuskapi_sdk.php`) exports the SDK class
