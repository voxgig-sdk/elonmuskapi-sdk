...
class FtCtrl extends ElonmuskapiControl
{
...
class FtClient
{
    ...
    private ElonmuskapiUtility $utility;

    public function __construct(ElonmuskapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): ElonmuskapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ElonmuskapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ElonmuskapiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ElonmuskapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ElonmuskapiConfig::make_feature($name);
    ...
    private static function buildUrl(ElonmuskapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ElonmuskapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ElonmuskapiResponse($fetched);
        }
    ...
    private function fail(ElonmuskapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ElonmuskapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
