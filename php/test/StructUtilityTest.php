...
// Elonmuskapi SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ElonmuskapiOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ElonmuskapiOmni::structfix($now, Struct::undef()))
                        !== json_encode(ElonmuskapiOmni::structfix($val, Struct::undef()))) {
