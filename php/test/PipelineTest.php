...
require_once __DIR__ . '/../elonmuskapi_sdk.php';
...
{
    private static function utility(): ElonmuskapiUtility
    {
        return new ElonmuskapiUtility();
    }
    ...
    private static function ctx(array $over = []): ElonmuskapiContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ElonmuskapiContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ElonmuskapiOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ElonmuskapiError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ElonmuskapiOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ElonmuskapiMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ElonmuskapiOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ElonmuskapiMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ElonmuskapiOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ElonmuskapiMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ElonmuskapiSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ElonmuskapiMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ElonmuskapiResponse([]),
            'result' => new ElonmuskapiResult([]),
        ]);
        [, $err] = ElonmuskapiMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ElonmuskapiSpec([]),
            'response' => null,
            'result' => new ElonmuskapiResult([]),
        ]);
        [, $err] = ElonmuskapiMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ElonmuskapiSpec(['step' => 's']),
            'response' => new ElonmuskapiResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ElonmuskapiResult([]),
        ]);
        [, $err] = ElonmuskapiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ElonmuskapiSpec(['step' => 's']),
            'response' => new ElonmuskapiResponse(self::resp(200, ['v' => 1])),
            'result' => new ElonmuskapiResult([]),
        ]);
        [, $err] = ElonmuskapiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ElonmuskapiSpec(['step' => 's']),
            'response' => new ElonmuskapiResponse(self::resp(200, ['v' => 2])),
            'result' => new ElonmuskapiResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ElonmuskapiMakeResponse::call($ctx);
    ...
    {
        $preset = new ElonmuskapiResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ElonmuskapiSpec([]),
            'response' => new ElonmuskapiResponse([]),
            'result' => new ElonmuskapiResult([]),
        ...
        [$got, $err] = ElonmuskapiMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ElonmuskapiResult([])]);
        [, $err] = ElonmuskapiMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ElonmuskapiSpec([]), 'result' => null]);
        [, $err] = ElonmuskapiMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ElonmuskapiSpec(['step' => 's']),
            'result' => new ElonmuskapiResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ElonmuskapiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ElonmuskapiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ElonmuskapiSpec(['step' => 's']),
            'result' => new ElonmuskapiResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ElonmuskapiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ElonmuskapiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ElonmuskapiResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ElonmuskapiSpec([]),
            'result' => new ElonmuskapiResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ElonmuskapiMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ElonmuskapiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ElonmuskapiError('boom', 'boom');
        $utility->fetcher = function (ElonmuskapiContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ElonmuskapiSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ElonmuskapiMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ElonmuskapiContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ElonmuskapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ElonmuskapiResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ElonmuskapiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ElonmuskapiContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ElonmuskapiSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ElonmuskapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ElonmuskapiResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ElonmuskapiSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ElonmuskapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ElonmuskapiMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ElonmuskapiMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ElonmuskapiResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ElonmuskapiDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ElonmuskapiResult(['ok' => false])]);
        $this->expectException(ElonmuskapiError::class);
        ElonmuskapiDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ElonmuskapiResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ElonmuskapiMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ElonmuskapiMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ElonmuskapiBaseFeature();
        $b = new ElonmuskapiBaseFeature();
        ElonmuskapiFeatureAdd::call($ctx, $a);
        ElonmuskapiFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ElonmuskapiBaseFeature
    {
        $f = new ElonmuskapiBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ElonmuskapiFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?ElonmuskapiSpec $spec): ElonmuskapiContext
    {
    ...
    private static function auth_bags(): ElonmuskapiSpec
    {
        return new ElonmuskapiSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        ElonmuskapiPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ElonmuskapiPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ElonmuskapiResponse(['headers' => null]),
            'result' => new ElonmuskapiResult([]),
        ]);
        ElonmuskapiResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ElonmuskapiResultBody::call($ctx);
