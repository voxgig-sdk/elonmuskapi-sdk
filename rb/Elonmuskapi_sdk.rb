...
require_relative 'Elonmuskapi_types'
...
class ElonmuskapiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ElonmuskapiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ElonmuskapiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ElonmuskapiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ElonmuskapiUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ElonmuskapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ElonmuskapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ElonmuskapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ElonmuskapiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ElonmuskapiError.new(
        "graphql_error", "ElonmuskapiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ElonmuskapiSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
