...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_random_article"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ElonmuskapiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "elonmuskapi_list",
		Description: "List records from Elonmuskapi. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "elonmuskapi_load",
		Description: "Load a single record from Elonmuskapi. " +
...
func runOp(client *sdk.ElonmuskapiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ElonmuskapiSDK, name string) (sdk.ElonmuskapiEntity, error) {
	switch strings.ToLower(name) {
	case "get_random_article":
		return client.GetRandomArticle(nil), nil
