...
[MCP](https://modelcontextprotocol.io) server exposing the Elonmuskapi SDK as
two agent tools — `elonmuskapi_list` and `elonmuskapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/elonmuskapi-mcp)
make build

# 2. Provide credentials via the environment
export ELONMUSKAPI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user elonmuskapi \
  -- /absolute/path/to/elonmuskapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./elonmuskapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// elonmuskapi_list: first page of records
{ "entity": "get_random_article" }
{ "entity": "get_random_article", "query": { } }

// elonmuskapi_load: one record by id
{ "entity": "get_random_article", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/elonmuskapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user elonmuskapi \
     -- "$PWD"/dist/*/elonmuskapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `elonmuskapi_list` and `elonmuskapi_load` tools now appear
   in new sessions. Ask the agent to *"list get_random_article using elonmuskapi"*
   and it calls `elonmuskapi_list` with `{"entity":"get_random_article"}`.
...
```sh
export ELONMUSKAPI_APIKEY=sk_live_xxx            # API key
export ELONMUSKAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `elonmuskapi_list` tool
...
### Call the `elonmuskapi_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `elonmuskapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `elonmuskapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ELONMUSKAPI_APIKEY` | API key sent with every request. |
| `ELONMUSKAPI_BASE` | Optional override of the API base URL. |
...
```sh
./elonmuskapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"elonmuskapi_load","arguments":{"entity":"get_random_article","query":{"id":1}}}}'
```
