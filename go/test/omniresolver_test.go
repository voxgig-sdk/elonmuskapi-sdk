...
import (
	...
	omni "github.com/voxgig-sdk/elonmuskapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ElonmuskapiError); is {
				out := map[string]any{
					"name":    "ElonmuskapiError",
...
func omniCtx(arg any, client *sdk.ElonmuskapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ElonmuskapiSDK); is {
			client = live
