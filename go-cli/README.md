...
boru-driven command-line client **and** interactive REPL for the Elonmuskapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/elonmuskapi-cli)
make build

# 2. See usage (words, entities, env vars)
./elonmuskapi-cli --help

# 3. Provide credentials once, via the environment
export ELONMUSKAPI_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./elonmuskapi-cli load 1 get_random_article            # {id:1} shorthand
./elonmuskapi-cli load '{id:1}' get_random_article       # explicit match map

# 5. Override the API base URL for a single call
ELONMUSKAPI_BASE=https://api.example.com ./elonmuskapi-cli load 1 get_random_article

# 6. No arguments -> interactive REPL
./elonmuskapi-cli
elonmuskapi> load 1 get_random_article
elonmuskapi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/elonmuskapi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/elonmuskapi-cli load 1 get_random_article
   ```
...
```sh
./elonmuskapi-cli load 1 get_random_article          # scalar shorthand for {id:1}
./elonmuskapi-cli load '{id:1}' get_random_article     # explicit match map
```
...
./elonmuskapi-cli load 1 get_random_article
```
...
Run with no arguments to open a REPL (prompt `elonmuskapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_article`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/elonmuskapi-cli`. |
...
(`./elonmuskapi-cli <expr>`) and interactively (the REPL), and expressions compose the
